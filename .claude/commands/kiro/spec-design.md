...
- Template conformance checklist (include verbatim in design):
  ...
  3. All client/server interactions use `next-safe-action`; client validation relies on React Hook Form + Zod, server validation via `.inputSchema()`.
  4. Hidden inputs for server-only data are prohibited; use `.bind()` arguments or action metadata instead.
  5. Actions avoid `try/catch`; rely on `handleServerError`. Cache invalidation (`revalidateTag`, etc.) is performed only inside actions.
  6. Usecases perform domain checks and call `returnValidationErrors` when validation fails; usecases never call cache APIs.
  7. Queries, mutations, and services are invoked exclusively through usecases. Direct invocation from components or actions is forbidden.
  8. RSC data fetching always goes through usecases (never queries/services directly).
