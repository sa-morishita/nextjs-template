...
- **Template alignment**: Default to the existing project template patterns. Avoid introducing new abstractions unless absolutely required; discuss and obtain approval during planning if template deviations are necessary. Design drafts must explicitly state adherence to the "Template Conformance Checklist" described below.
...
- Template conformance checklist (include verbatim in design):
  1. UI enforces container/presentational separation; page-level RSC orchestrates containers.
  2. `page.tsx` remains an RSC that wraps data retrieval in `Suspense` with skeleton fallbacks and invokes `getSession()` server-side; containers must not call `getSession` directly because the helper already handles redirects.
  3. **Container Directory Structure (Required)**: Always create a subdirectory under `_containers` containing `index.tsx` (re-export), `container.tsx` (Server Component), and `presentational.tsx` (Server Component). This enables parallel data fetching with multiple Suspense boundaries in page.tsx (Next.js App Router best practice). Place Client Components requiring interactivity (forms, etc.) in `_components`.
     ```
     _containers/
       └── {feature-name}/
           ├── index.tsx          # export { FeatureContainer } from './container';
           ├── container.tsx      # async Server Component (data fetching + usecase calls)
           └── presentational.tsx # Server Component (UI layout, imports Client Components from _components)
     _components/
       └── {feature}-form.tsx     # 'use client' (forms, interactions, etc.)
     ```
  4. All client/server interactions use `next-safe-action`; client validation relies on React Hook Form + Zod, server validation via `.inputSchema()`.
  5. Hidden inputs for server-only data are prohibited; use `.bind()` arguments or action metadata instead.
  6. Actions avoid `try/catch`; rely on `handleServerError`. Cache invalidation (`revalidateTag`, etc.) is performed only inside actions.
  7. Usecases perform domain checks and call `returnValidationErrors` when validation fails; usecases never call cache APIs.
  8. Queries, mutations, and services are invoked exclusively through usecases. Direct invocation from components or actions is forbidden.
  9. RSC data fetching always goes through usecases (never queries/services directly).

**UI-First Implementation Guidance (for spec-tasks generation)**:

When generating vertical slice tasks, ensure Task X.1 includes:
1. **Container/Presentational Structure**:
   - `page.tsx` (RSC) → `_containers/{feature-name}/container.tsx` (Server Component) → `_containers/{feature-name}/presentational.tsx` (Server Component) → `_components/*-form.tsx` (Client Component)
   - **CRITICAL**: Never place files directly under `_containers`; always create a subdirectory first
   - Implement `presentational.tsx` as a Server Component; place Client Components requiring interactivity in `_components`

2. **Production Client Components**:
   - Use `useHookFormAction` from `@next-safe-action/adapter-react-hook-form/hooks`
   - Implement toast-based error handling with `sonner`
   - Full Zod validation with React Hook Form

3. **Server Action Stubs**:
   - Create action files in `lib/actions` with production structure
   - Use `next-safe-action` with `.metadata()`, `.inputSchema()`, `.action()`
   - Implement stub logic: `console.log()` + `setTimeout()` + mock response
   - Add TODO comments: `// TODO: Task X.2 - Implement real logic`
...
This ensures Task X.2-X.3 only modify server action internals, not UI structure.
